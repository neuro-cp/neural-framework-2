...
def test_learning_input_bundle_order_invariant():
    ...
    pattern = PatternRecord(pattern_counts={sig_a: 4, sig_b: 1})
    ...
    # Same content, deliberately shuffled ordering
    bundle_b = builder.build(
        replay_id="r1",
        episodes=[ep2, ep1],
        semantic_records=[sem2, sem1],
        pattern_record=pattern,
        semantic_episode_pairs=[("sem:alpha", 1), ("sem:alpha", 2)],
    )
