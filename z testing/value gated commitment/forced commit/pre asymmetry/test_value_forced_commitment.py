...
# -------------------------
SMALL_POKE = 1.0
COINCIDENCE_POKE = 1.5   # extra push at value moment only
...
# -------------------------
VALUE_REGION = "vta"     # biologically correct
VALUE_MAG = 0.45         # intentionally strong (calibration)
VALUE_STEP = 12          # arrives after evidence has built
...
# ============================================================
# MAIN — FORCED COMMIT CALIBRATION
# ============================================================

log("=== BEGIN FORCED COMMIT CALIBRATION TEST ===")
init_csvs()
...
for i in range(1, INTEGRATION_STEPS + 1):
    ...
    if i == VALUE_STEP:
        ...
        poke("striatum", COINCIDENCE_POKE)
        poke(VALUE_REGION, VALUE_MAG)

    time.sleep(DT * 25)

    parsed = parse_striatum_diag(send("striatum_diag"))
    if not parsed:
        continue
